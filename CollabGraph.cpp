 ...
 **   April 2021: Lucas Maley 
 **       Implemented report_path and get_vertex_neighbors
 **
 **/
...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist &artist) const
{
    ...
    //gets neighbors and stores them in vector of edges
    vector<Edge> edges = graph.at(artist.get_name())->neighbors;
    
    //adds neighbors to result vector
    for (unsigned long i = 0; i < edges.size(); i++) {
        result.push_back(edges.at(i).neighbor);
    }
                                            ...
                                            const Artist &dest) const
{
    ...
    Artist curr_artist = dest;
    bool quit = false;
    
    //iterates backwards from destination through the predecessor,
    //adding each artist in path to a stack
    while(curr_artist != source && not quit) {
        path.push(curr_artist);
        if (graph.at(curr_artist.get_name())->predecessor == nullptr) {
            quit = true;
        }
        curr_artist = get_predecessor(curr_artist);
    }
    path.push(curr_artist); //adds last artist to path stack
